...
class PruneCriteria:
    def __init__(self, min_error_improvement:float=0.00001, min_samples_leaf:int=1, min_samples_split=1, max_height:int=np.iinfo(int).max,error_tolerance:float=1e-16):
        assert(min_samples_leaf>0)
        assert(min_error_improvement>0)
        assert(max_height>0)    
    ...
    def params(self):
        return {"min_error_improvement":self.min_error_improvement,
    ...
    def post_split_prune(self,tree:Tree,best_column:ColumnSplitterResult):
        ...
        return error_improvement<self.min_error_improvement

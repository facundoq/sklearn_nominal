...
def get_nominal_tree_regressor(x:pd.DataFrame,y:np.ndarray):
    ...
    return SKLearnRegressionTree(criterion="std",max_depth=max_height,min_samples_leaf=min_samples_leaf,min_samples_split=min_samples_split,min_error_decrease=min_error_improvement,splitter=4)
...
def get_sklearn_tree(x:pd.DataFrame,y:np.ndarray):
    ...
    min_samples_split = min_samples_leaf
    min_error_improvement = 0.05*y.std()
    model = sklearn.tree.DecisionTreeRegressor(max_depth=max_height,min_samples_leaf=min_samples_leaf,min_samples_split=min_samples_split,min_impurity_decrease=min_error_improvement,criterion="squared_error")
...
def test_performance_similar_sklearn(at_most_percent=1.5,dataset_names=dataset_names):
    ...
    for dataset in tqdm(datasets,desc=f"Datasets"):
        nominal_results = train_test_classification_model("NominalTree",get_nominal_tree_regressor,dataset)
        ...
        for set in ["Train","Test"]:
            ...
            assert percent<=at_most_percent, f"{set} score of nominal tree ({nominal:.2f}) should be at most {at_most_percent*100:.2f}% of sklearn.tree ({numeric:.2f}) on dataset {nominal_results["Dataset"]}, was {percent*100:.2f}% instead."

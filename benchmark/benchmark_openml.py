...
def benchmark(model_generator:typing.Callable)->pd.DataFrame:
    ...
    for i,task_id in enumerate(benchmark_suite.tasks):  # iterate over all tasks
        ...
        if m>1000:
            ...
            x = pd.DataFrame(data=x,columns=[f"v_{i}={v:.2f}" for i,v in enumerate(pca.explained_variance_ratio_)])
        else:
            pass
...
def plot_results(df:pd.DataFrame,platform:str):
    ...
    for y in ["train_accuracy","train_time","test_time"]:
        plot = lp.ggplot(df, lp.aes(x='dataset', y=y, color='model')) + common_options + x_scale + axis_font
...
def compute_results(platform,force=False):
    table_path = basepath /f"openml_cc18_{platform}.csv"
    md_path = basepath /f"openml_cc18_{platform}.md"
    if not table_path.exists() or force:
...
if __name__ == "__main__":
    ...
    df = compute_results(platform,force=False)
    print(df)
    
    plot_results(df,platform)
    
    with open(basepath/f"{platform}_benchmark.md","w") as f:
